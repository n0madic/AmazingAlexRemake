...
#
#   tools/build_linux_docker.sh [--assets build/assets] [--image ubuntu:24.04] [--build build-linux-docker] [--icon PNG]
...
docker run --rm \
    ...
    -e DEBIAN_FRONTEND=noninteractive "$IMAGE" bash -euo pipefail -c '
        ...
        cmake -S /src -B /build -G Ninja -DCMAKE_BUILD_TYPE=Release -DAA_ASSETS=/assets > /build/configure.log
