...
typedef struct {
    ...
    bool activityResumed;               // Lifecycle state; unlike input focus, safe to use for blocking
    bool contextRebindRequired;         // Used to know context rebind required
    bool useNativeWindowSize;           // InitWindow(0, 0): let Android own the surface dimensions
...
static bool UpdateFramebufferForCurrentWindow(struct android_app *app); // Apply a live Android surface-size change
...
void PollInputEvents(void)
{
    ...
    // Match the desktop backends: a resize reported while polling events belongs to the
    // following frame. Without this reset a single fold/unfold would relayout the app forever.
    ...
    // Input focus can be lost transiently while a resumed foldable Activity moves to its other
    // display. Blocking on appEnabled here prevented the next resized frame from ever being drawn.
    // Pause/resume is the lifecycle boundary that should actually suspend the render loop.
    while ((pollResult = ALooper_pollOnce((platform.activityResumed || FLAG_IS_SET(CORE.Window.flags, FLAG_WINDOW_ALWAYS_RUN))? 0 : -1, NULL, &pollEvents, ((void **)&platform.source)) > ALOOPER_POLL_TIMEOUT))
    {
        // Process this event
    ...
    // A fold/unfold is not guaranteed to arrive as APP_CMD_CONFIG_CHANGED: vendors may resize an
    // existing surface or replace it through TERM/INIT_WINDOW. EGL is the source of truth in every
    // case, and querying two integers once per frame is cheap compared with the buffer swap itself.
    UpdateFramebufferForCurrentWindow(platform.app);
}
...
static int InitGraphicsDevice(void)
{
    ...
    platform.useNativeWindowSize = ((CORE.Window.screen.width == 0) || (CORE.Window.screen.height == 0));
    SetupFramebuffer(CORE.Window.display.width, CORE.Window.display.height);

    // InitWindow(0, 0) means the application follows Android's window. Keeping width/height at zero
    // is essential on foldables: an explicit launch-size buffer remains pinned and SurfaceFlinger
    // stretches it when the Activity moves between the cover and inner displays.
    ANativeWindow_setBuffersGeometry(platform.app->window,
        platform.useNativeWindowSize? 0 : CORE.Window.render.width,
        platform.useNativeWindowSize? 0 : CORE.Window.render.height,
        displayFormat);
...
static void AndroidCommandCallback(struct android_app *app, int32_t cmd)
{
    switch (cmd)
    {
            ...
            if (app->window != NULL)
            {
                if (platform.contextRebindRequired)
                {
                    ...
                    ANativeWindow_setBuffersGeometry(app->window,
                        platform.useNativeWindowSize? 0 : CORE.Window.render.width + CORE.Window.renderOffset.x,
                        platform.useNativeWindowSize? 0 : CORE.Window.render.height + CORE.Window.renderOffset.y,
                        displayFormat);
        ...
        case APP_CMD_PAUSE:
        {
            platform.activityResumed = false;
        } break;
        ...
        case APP_CMD_CONFIG_CHANGED:
        {
            ...
            // Foldables can change the NativeActivity's bounds while retaining both the Activity and
            // its EGL surface. The old Android backend left the old buffer geometry and viewport in
            // place, so SurfaceFlinger stretched the last-sized game frame until a restart. A mobile
            // window is always full-display here (InitWindow(0, 0)), therefore the new native bounds
            // are both the desired screen size and the render-target size. PollInputEvents also checks
            // EGL every frame because some vendor fold transitions do not deliver this command.
            UpdateFramebufferForCurrentWindow(app);
        } break;
        case APP_CMD_WINDOW_RESIZED:
        case APP_CMD_WINDOW_REDRAW_NEEDED:
        case APP_CMD_CONTENT_RECT_CHANGED:
        {
            // native_app_glue defines these specifically for a live NativeWindow resize/redraw.
            // PollInputEvents returns to the application after processing them, and the next frame
            // uses the new EGL size instead of leaving SurfaceFlinger to stretch the old front buffer.
            UpdateFramebufferForCurrentWindow(app);
        } break;
        default: break;
    }
...
// Refresh raylib's public screen size and its GL viewport after Android changes an existing
// NativeActivity surface (notably a Fold cover/display transition). This is deliberately separate
// from APP_CMD_INIT_WINDOW: a configuration change commonly keeps the EGL context alive, so running
// InitGraphicsDevice again would discard the application's GPU resources.
static bool UpdateFramebufferForCurrentWindow(struct android_app *app)
{
    if ((app == NULL) || (app->window == NULL) || (platform.device == EGL_NO_DISPLAY) ||
        (platform.surface == EGL_NO_SURFACE) || (platform.context == EGL_NO_CONTEXT)) return false;

    // ANativeWindow_getWidth() can still describe the old explicitly sized buffer while Android is
    // replacing it. EGL_WIDTH/EGL_HEIGHT describe the drawable we will actually present into.
    EGLint width = 0;
    EGLint height = 0;
    if ((eglQuerySurface(platform.device, platform.surface, EGL_WIDTH, &width) == EGL_FALSE) ||
        (eglQuerySurface(platform.device, platform.surface, EGL_HEIGHT, &height) == EGL_FALSE)) return false;
    if ((width <= 0) || (height <= 0) ||
        ((width == (int)CORE.Window.screen.width) && (height == (int)CORE.Window.screen.height))) return false;
    ...
    // SetupViewport() is internal to rcore.c, but this platform file is compiled into the same
    // translation unit. Keep this sequence exactly in sync with that helper.
