...
: b2Joint(def)
{
	...
	// AMAZING ALEX LOCAL CHANGE: the shipped trunk does not normalise the axis (2.1.2 behaviour);
	// the game passes non-unit axes (boxing glove).
...
// (spring 2011) linked into the original game: the 2.1.2 prismatic joint (3x3 block solver with the
// old K layout) plus the trunk's "k22 == 0 -> 1" guard, expressed on 2.2.1's b2SolverData.
// Expression shapes follow the decompiled original — keep them for bit identity.
...
void b2PrismaticJoint::SolveVelocityConstraints(const b2SolverData& data)
{
	...
	if (m_enableLimit && m_limitState != e_inactiveLimit)
	{
		...
		b2Vec2 Cdot(Cdot1.x, Cdot1.y);
		b2Vec3 Cdot3(Cdot1.x, Cdot1.y, Cdot2);

		b2Vec3 f1 = m_impulse;
		b2Vec3 df =  m_K.Solve33(-Cdot3);
		m_impulse += df;
		...
		// f2(1:2) = invK(1:2,1:2) * (-Cdot(1:2) - K(1:2,3) * (f2(3) - f1(3))) + f1(1:2)
		b2Vec2 b = -Cdot - (m_impulse.z - f1.z) * b2Vec2(m_K.ez.x, m_K.ez.y);
...
{
	b2Vec2 c1 = data.positions[m_indexA].c;
	float32 a1 = data.positions[m_indexA].a;
	b2Vec2 c2 = data.positions[m_indexB].c;
	float32 a2 = data.positions[m_indexB].a;
	...
	b2Vec2 r1 = b2Mul(R1, m_localAnchorA - m_localCenterA);
	b2Vec2 r2 = b2Mul(R2, m_localAnchorB - m_localCenterB);
	b2Vec2 d = c2 + r2 - c1 - r1;

	if (m_enableLimit)
	{
		m_axis = b2Mul(R1, m_localXAxisA);
	...
	data.positions[m_indexA].c = c1;
	data.positions[m_indexA].a = a1;
	data.positions[m_indexB].c = c2;
